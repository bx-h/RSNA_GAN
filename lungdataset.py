...
def print_dataset(dataset, print_time):
    ...
    counter = Counter()
    labels = []
    for index, (img, label, _) in enumerate(dataset):
...
def normal_dataset(dataset, name):
    paths = []
    for index, (img, label, path) in enumerate(dataset):
        if label is 0:
            p = path.split('/')[-1]
            paths.append(p)
...
if __name__ == "__main__":
    ...
    # normal_dataset(dataset, '/data15/boxi/anomaly detection/data/train-normal-data')
